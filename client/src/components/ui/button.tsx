...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading = false, loadingText, children, disabled, ...props }, ref) => {
    ...
    const isIconOnly = size === 'icon' || (!children || (typeof children === 'object' && React.isValidElement(children)));
    
    // Warn if icon-only button doesn't have aria-label
    if (isIconOnly && !props['aria-label'] && !props['aria-labelledby']) {
      console.warn('Icon-only button should have aria-label or aria-labelledby');
    }
